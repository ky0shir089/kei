    ...
    >
      <template v-slot:top>
        <v-toolbar flat dense color="grey lighten-2">
          <v-toolbar-title>Setup Webinar Level</v-toolbar-title>
...
export default {
  ...
  data: () => ({
    headers: [
      { text: "ID", value: "id" },
      { text: "Webinar Level", value: "webinar_level" },
  ...
  computed: {
    ...
    formTitle() {
      return this.editedIndex === -1 ? "Tambah Level" : "Edit Level";
    },
  },
  methods: {
    ...mapActions({
    ...
    async saveMaster() {
      ...
      formData.set("webinar_level", this.form1.webinar_level.toUpperCase());
      formData.set("created_by", this.user.id);

      await this.axios
        .post("/setup-webinar/v1/webinar_level", formData, {
          headers: {
            Authorization: "Bearer " + this.user.api_token,
          },
        })
    ...
    async updateMaster() {
      ...
      await this.axios
        .put(
          "/setup-webinar/v1/webinar_level",
          {
            id: this.form1.id,
            webinar_level: this.form1.webinar_level.toUpperCase(),
