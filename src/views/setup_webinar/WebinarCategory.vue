    ...
    >
      <template v-slot:top>
        <v-toolbar flat dense color="grey lighten-2">
          <v-toolbar-title>Setup Webinar Category</v-toolbar-title>
          ...
          <v-row dense>
            <v-col cols="12">
              <v-form ref="form" v-model="valid1">
                <v-text-field
                  v-model="form1.webinar_category"
                  label="Webinar Kategori"
                  outlined
                  dense
                  :rules="required"
                ></v-text-field>
...
export default {
  ...
  data: () => ({
    headers: [
      { text: "ID", value: "id" },
      { text: "Webinar Category", value: "webinar_category" },
  ...
  methods: {
    ...
    async saveMaster() {
      ...
      formData.set("webinar_category", this.form1.webinar_category.toUpperCase());
      formData.set("created_by", this.user.id);

      await this.axios
        .post("/setup-webinar/v1/webinar_category", formData, {
          headers: {
            Authorization: "Bearer " + this.user.api_token,
          },
        })
    ...
    async updateMaster() {
      ...
      await this.axios
        .put(
          "/setup-webinar/v1/webinar_category",
          {
            id: this.form1.id,
            webinar_category: this.form1.webinar_category.toUpperCase(),
