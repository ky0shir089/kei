    ...
    >
      <template v-slot:top>
        <v-toolbar flat dense color="grey lighten-2">
          <v-toolbar-title>Setup Alasan Mengikuti Webinar</v-toolbar-title>
...
export default {
  ...
  computed: {
    ...
    formTitle() {
      return this.editedIndex === -1 ? "Tambah Alasan" : "Edit Alasan";
    },
  },
  methods: {
    ...mapActions({
    ...
    async saveMaster() {
      ...
      formData.set("reason", this.form1.reason.toUpperCase());
      formData.set("created_by", this.user.id);

      await this.axios
        .post("/setup-webinar/v1/reason", formData, {
          headers: {
            Authorization: "Bearer " + this.user.api_token,
          },
        })
    ...
    async updateMaster() {
      ...
      await this.axios
        .put(
          "/setup-webinar/v1/reason",
          {
            id: this.form1.id,
            reason: this.form1.reason.toUpperCase(),
