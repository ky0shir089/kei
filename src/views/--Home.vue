    ...
    <v-container fluid>
      <div style="max-width: 1240px; margin: 0 auto !important">
        <v-carousel cycle show-arrows hide-delimiter-background height="400">
          <v-carousel-item
            v-for="item in webinarSesi.slice(0, 4)"
            :key="item.id"
            :src="getImage(item.webinar_banner)"
            reverse-transition="fade-transition"
            transition="fade-transition"
            :to="'/workshop/' + item.webinar_class.toLowerCase() + '/' + item.webinar_name.toLowerCase().replaceAll(' ', '-') + '-' + item.id"
            max-width="1240"
            max-height="400"
            align="center"
            contain
        ...
        <v-row>
          <v-col cols="12" sm="9" order-sm="1" order="2">
            <div v-if="notes.length > 0">
              <v-row>
                <v-col cols="12" sm="4" v-for="item in notes" :key="item.id">
                  <v-card rounded>
                    <v-img :src="getImage(item.webinar_banner)" height="200" contain></v-img>

                    <v-container fluid>
                      <div class="text-subtitle-2 font-weight-bold">{{ item.webinar_name }}</div>
                      ...
                      <v-list three-line>
                        <v-list-item>
                          <v-list-item-avatar>
                            <v-img
                              :src="getImage(item.mentor_avatar)"
                              v-if="item.mentor_avatar != null"
                            ></v-img>
                    ...
                    <v-card-actions>
                      <v-btn
                        ...
                        block
                        @click="rincian(item)"
                        v-if="item.webinar_class == 'PAKET'"
                      >Rincian Materi</v-btn>
                      <v-btn color="blue" dark block @click="workshop(item)" v-else>Rincian Materi</v-btn>
                    </v-card-actions>
                  </v-card>
                </v-col>
              </v-row>
          ...
          <v-col cols="12" sm="3" order-sm="2" order="1">
            <v-date-picker
              v-model="date"
              full-width
              :events="events"
      ...
      await this.axios
        ...
        .then((response) => {
          ...
          if (this.webinarSpecial.length > 0) {
            if (this.webinarSpecial.webinar_class == 'SESI') {
              this.workshop(this.webinarSpecial[0])
            } else {
              this.rincian(this.webinarSpecial[0])
            }
          } else {
            this.setAlert({
              status: true,
