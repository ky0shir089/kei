<template>
  <v-container fluid>
    <div style="max-width: 1240px; margin: 0 auto !important">
      <v-card>
        ...
        <v-container fluid>
          <v-radio-group v-model="form.reason_id" class="mt-0" :rules="[rules.required]">
            <v-radio v-for="item in reasons" :key="item.id" :label="item.reason" :value="item.id"></v-radio>
          </v-radio-group>
        </v-container>
      </v-card>
        ...
        <v-card>
          ...
          <v-container fluid>
            <v-radio-group v-model="form.hobby_id" class="mt-0" :rules="[rules.required]">
              <v-radio v-for="item in hobbies" :key="item.id" :label="item.hobby" :value="item.id"></v-radio>
            </v-radio-group>
          </v-container>
        </v-card>
      </v-form>
      ...
      <v-btn color="primary" block :disabled="!valid" :loading="loading" @click="check">Update</v-btn>
    </div>
  </v-container>
</template>
...
export default {
  ...
  methods: {
    ...
    async getProfile() {
      await this.axios
        ...
        .then((response) => {
          ...
          if (data.length > 0) {
            this.form = {
              reason_id: this.profiles[0].reason_id,
    ...
    async survey() {
      ...
      let formData = new FormData();
      formData.set("id", this.profiles[0].id);
      ...
      await this.axios
        .post("/peserta/v1/update-survey", formData, {
          headers: {
            Authorization: "Bearer " + this.user.api_token,
          },
        })
